...
def _topk(scores, K=40):
    ...
    def t(x, dtype=torch.float, device='cpu'):
      return torch.tensor(x, dtype=dtype, device=device)


    topk_inds = topk_inds % (t([height], torch.int64) * t([width], torch.int64))
    topk_ys   = (topk_inds / t([width], torch.float)).int().float()
    topk_xs   = (topk_inds % t([width], torch.int64)).int().float()
...
def find4ps(bbox, x, y):
    ...
    xs = torch.tensor([bbox[0], bbox[2], bbox[4], bbox[6]], dtype=torch.float)
    ys = torch.tensor([bbox[1], bbox[3], bbox[5], bbox[7]], dtype=torch.float)
